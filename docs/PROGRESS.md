...
| 03 — Omakeez overlay | In progress (rebrand complete) | 529b300 | Omakeez is now the plugin identity, runtime namespace, bridge protocol, user-facing name, and repository name. The setup panel uses a calm, decision-led safe state with compact Symlink/Git supporting checks, an adaptive green/amber safety verdict, and a single keyboard-accessible enable CTA. The raw diff and bridge source are lightweight on-demand transparency links; manual/recheck actions appear only when automatic setup needs review. The panel now sizes to visible content and expands only when the exact diff is requested, with the card's own inset included so the primary CTA remains fully visible. The enabled view is reduced to integration, delay, and fullscreen settings with concise guidance; gamification, local data, and the Dojo entry are removed. The bridge now has an Omakeez-specific global state name, so a prior Keybind Dojo session cannot suppress its callbacks after rebrand. Next: remove the remaining persistence/gamification implementation and bar badges, then reduce the panel to integration, fullscreen behavior, security documentation, and catalog access. See `UX_POLISH_02_OMAKEEZ_OVERLAY.md`. |
