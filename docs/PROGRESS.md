...
| 03 — Omakeez overlay | In progress (rebrand complete) | 7bf197f | Omakeez is now the plugin identity, runtime namespace, bridge protocol, user-facing name, and repository name. The setup panel now explains the safety review and one-block diff replacement, removes catalog-progress noise, and presents Enable as a selected keyboard-focusable primary CTA activated by Enter. Safety details now use explicit Symlink check, Git check, and final Safety sections, with path details only for mismatches, adaptive green/amber states, and manual-snippet guidance for fallback paths. The diff explanation now describes the normal Hyprland bridge addition and links directly to the public bridge source. The panel hierarchy now separates review information, the primary next step, and optional tools. Next: remove activity persistence/gamification and bar badges, then reduce the panel to integration, fullscreen behavior, security documentation, and catalog access. See `UX_POLISH_02_OMAKEEZ_OVERLAY.md`. |
