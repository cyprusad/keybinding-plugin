...
| 03 — Omakeez overlay | In progress (rebrand complete) | 88f3ae8 | Omakeez is now the plugin identity, runtime namespace, bridge protocol, user-facing name, and repository name. The setup panel now uses visual-guide wording, cohesive action buttons, and theme-aware green/red integration diff lines. Next: remove activity persistence/gamification and bar badges, then reduce the panel to integration, fullscreen behavior, security documentation, and catalog access. See `UX_POLISH_02_OMAKEEZ_OVERLAY.md`. |
