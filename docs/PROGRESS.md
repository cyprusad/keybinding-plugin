...
| 02 — Privacy-first guide coverage | In progress (phase 1 ready for live gate) | 36461e2 | `SUPER`, `CTRL`, `ALT`, and `SHIFT` can now each root the existing hold-to-reveal guide under the current integration consent. The first-held modifier remains the guide root until release, and added modifiers select the exact chord lane. Per-binding selection, collision exclusion, explicit extended opt-in, and later telemetry removal remain separate phases. See `PRIVACY_FIRST_GUIDES.md`. |
