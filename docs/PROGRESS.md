...
| 03 — Omakeez overlay | In progress (rebrand complete) | b35a729 | Omakeez is now the plugin identity, runtime namespace, bridge protocol, user-facing name, and repository name. The setup panel uses a calm, decision-led safe state with compact Symlink/Git supporting checks, an adaptive green/amber safety verdict, and a single keyboard-accessible enable CTA. The raw diff and bridge source are lightweight on-demand transparency links; manual/recheck actions appear only when automatic setup needs review. The panel now sizes to visible content and expands only when the exact diff is requested, with the card's own inset included so the primary CTA remains fully visible. Its onboarding and enabled views now share all-caps group headers, muted field labels, and native panel separators; idle setup status text is omitted. The enabled view is reduced to visual-guide controls and integration: delay, fullscreen behavior, and independently persisted `SUPER`, `SHIFT`, `CTRL`, and `ALT` guide-root toggles. Disabling a root suppresses only its overlay; registered keyboard shortcuts continue to work normally. Gamification, local data, and the Dojo entry are removed from the panel. The bar icon likewise has no streak badge and uses plain operational tooltip text. User-facing setup and overlay language consistently describes a visual guide rather than tracking. The bridge now has an Omakeez-specific global state name, so a prior Keybind Dojo session cannot suppress its callbacks after rebrand, and it lazily activates as soon as its first catalog generation finishes. Next: remove the remaining persistence/gamification implementation and bar badges, then reduce the panel to integration, fullscreen behavior, security documentation, and catalog access. See `UX_POLISH_02_OMAKEEZ_OVERLAY.md`. |
